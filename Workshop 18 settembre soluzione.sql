...
Step 5 
Creare una procedura con gestione degli errori e delle transazioni 
che cancelli le righe esistenti nella CleanedOrders 
e poi esegua la INSERT dei dati.

Step 6
Modificare lo step 5 in modo che le righe 
cancellate vengano scritte nella tabella CleanedOrdersLog.
...
/*ATTENZIONE ALLE DIFFERENZE TRA DELETE E TRUNCATE!*/

/*ATTENZIONE AL LIVELLO DI ISOLAMENTO READ COMMITTED SNAPSHOT.
IMPATTA SULLA DECISIONE DI SALVARE I DATI IN UNA TABELLA TEMPORANEA
PRIMA DELL'APERTURA DELLA TRANSAZIONE*/

/*ATTENZIONE A USARE LE TRANSAZIONI SOLO QUANDO RICHIESTO DAL
PROCESSO (SOPRATTUTTO SE DEVO CANCELLARE E INSERIRE TANTI RECORD)*/
...
go
/*Step 5*/
CREATE OR ALTER PROCEDURE projectwork.LoadCleanedOrders
AS
SET XACT_ABORT ON
SET NOCOUNT ON
BEGIN
    ...
    BEGIN TRAN
        ...
        TRUNCATE TABLE CorsoSQL.projectwork.CleanedOrders;
...
BEGIN
    ...
    BEGIN TRAN
        ...
        DELETE 
        FROM   CorsoSQL.projectwork.CleanedOrders
        OUTPUT deleted.RowID,deleted.OrderID,deleted.OrderDate,
                     deleted.ShipDate,deleted.ShipMode
                   ,deleted.CustomerID,deleted.CustomerName,deleted.Segment,deleted.Country
                   ,deleted.City,deleted.State,deleted.PostalCode,deleted.Region,deleted.ProductID
                   ,deleted.Catery,deleted.SubCatery,deleted.ProductName,deleted.Sales
                   ,deleted.Quantity,deleted.Discount,deleted.Profit,deleted.DataInizioValidità
                   ,@data_inizio_validita as DataFineValidità
        INTO   CorsoSQL.projectwork.CleanedOrdersLog;
...
/*Test
select * from projectwork.CleanedOrders 
select * from projectwork.CleanedOrderslog where rowid = 5069
order by DataInizioValidità
exec projectwork.LoadCleanedOrders
*/